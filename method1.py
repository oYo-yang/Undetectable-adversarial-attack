...
def attacker(func, numPix, bounds, success_fun, img, maxiter=400):
    ...
    def bound(x_in, imgs):
        ...
        xs = (x_in * bounds_diff + lb).astype(int)
        ...
        x_in = (x_in2 - lb)/bounds_diff.astype(float)
...
def plot_image(image, label_true=None, class_names=None, label_pred=None):
    ...
    plt.subplot(1, 2, 1)  # 将画板分为1行两列，本幅图位于第一个位置
    plt.grid()
    ...
    plt.title('Pre-attack:'+class_names[label_true] + "(" + str(
                round(prior_confidence*100, 1)) + "%)", y=-0.2)
    plt.imshow(x_test[6706].astype(np.uint8))
    plt.xticks([])  # Remove ticks from the plot
    plt.yticks([])

    plt.subplot(1, 2, 2)  # 将画板分为1行两列，本幅图位于第二个位置

    labels_pred_name = class_names[label_pred]
    print('Post-attack:' + labels_pred_name + "(" + str(
                round(post_confidence*100, 1)) + " %)")
    plt.title('Post-attack:' + labels_pred_name + "(" + str(
                 round(post_confidence*100, 1)) + " %)", y=-0.2)
    plt.axis('off')
    plt.margins(0,0)
...
def attack(img_id, model, target=None, pixel_count=1, verbose=False, plot=False):
    ...
    attack_result = attacker(predict_fn, pixel_count, bounds, callback_fn, x_test[img_id]) #返回要修改像素的r,c,rgb
